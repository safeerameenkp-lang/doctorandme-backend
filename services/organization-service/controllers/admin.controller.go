...
func ListRoles(c *gin.Context) {
	// Only show roles relevant to clinic staff
	rows, err := config.DB.Query(`
		SELECT id, name, description, is_system_role 
		FROM roles 
		WHERE is_active = true 
		  AND name IN ('receptionist', 'pharmacist', 'lab_technician', 'billing_staff', 'nurse')
		ORDER BY name ASC
	`)
	if err != nil {
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &name, &description, &isSystemRole)
		if err != nil {
			continue
		}
...
func CreateStaff(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to hash password")
		return
	}
	...
	var staffID string
	err = tx.QueryRow(`
        INSERT INTO staff (user_id, clinic_id, staff_type, department, joining_date, permissions)
        VALUES ($1, $2, $3, $4, $5, $6) RETURNING id
    `, userID, input.ClinicID, input.StaffType, input.Department, input.JoiningDate, permissionsJSON).Scan(&staffID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create staff record")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, fmt.Sprintf("Failed to find role: %s. Please use a valid staff type like 'receptionist', 'pharmacist', 'lab_technician', or 'billing_staff'.", input.StaffType))
		return
	}
...
func GetClinicStaff(c *gin.Context) {
	...
	query := `
        SELECT s.id, s.user_id, u.first_name, u.last_name, u.email, u.username, u.phone,
               s.staff_type, s.department, s.joining_date, s.permissions, s.is_active, s.created_at
        FROM staff s
	...
	if roleFilter != "" {
		query += fmt.Sprintf(" AND s.staff_type = $%d", argIndex)
	...
	for rows.Next() {
		...
		var id, userID, firstName, username, staffType string
		var lastName, email, phone, department, joiningDate, permissionsJSON sql.NullString
		var isActive bool
		var createdAt time.Time

		err := rows.Scan(&id, &userID, &firstName, &lastName, &email,
			&username, &phone, &staffType, &department, &joiningDate, &permissionsJSON,
			&isActive, &createdAt)
		if err != nil {
			continue
		}
		...
		if joiningDate.Valid {
			s["joining_date"] = joiningDate.String
		}
		s["permissions"] = permissionsJSON.String
...
func GetStaffDetails(c *gin.Context) {
	...
	err := config.DB.QueryRow(`
        ...
        FROM staff s
        JOIN users u ON s.user_id = u.id
        WHERE s.clinic_id = $1 AND s.id = $2
    `, clinicID, staffID).Scan(&id, &userID, &firstName, &lastName, &email,
		&username, &phone, &staffType, &department, &joiningDate, &permissionsJSON,
		&isActive, &createdAt)
	if err != nil {
		if err == sql.ErrNoRows {
			middleware.SendNotFoundError(c, "staff member")
			return
		}
		middleware.SendDatabaseError(c, "Failed to fetch staff details")
		return
	}
...
type UpdateStaffRoleInput struct {
	StaffType   string   `json:"staff_type" binding:"required,oneof=clinic_admin receptionist doctor lab_technician pharmacist billing_staff"`
	Permissions []string `json:"permissions"`
}
...
func UpdateStaffRole(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to update staff role")
		return
	}
...
type UpdateStaffInput struct {
	...
	StaffType   *string  `json:"staff_type" binding:"omitempty"`
	Department  *string  `json:"department"`
	JoiningDate *string  `json:"joining_date"`
	Permissions []string `json:"permissions"`
...
func UpdateStaff(c *gin.Context) {
	...
	if input.FirstName != nil {
		userUpdates = append(userUpdates, fmt.Sprintf("first_name = $%d", argIdx))
		userArgs = append(userArgs, *input.FirstName)
		argIdx++
	}
	if input.LastName != nil {
		userUpdates = append(userUpdates, fmt.Sprintf("last_name = $%d", argIdx))
		userArgs = append(userArgs, *input.LastName)
		argIdx++
	}
	if input.Email != nil {
		userUpdates = append(userUpdates, fmt.Sprintf("email = $%d", argIdx))
		userArgs = append(userArgs, *input.Email)
		argIdx++
	}
	if input.Phone != nil {
		userUpdates = append(userUpdates, fmt.Sprintf("phone = $%d", argIdx))
	...
	if input.Password != nil {
		...
		userUpdates = append(userUpdates, fmt.Sprintf("password_hash = $%d", argIdx))
		userArgs = append(userArgs, string(passHash))
		argIdx++
	}

	if len(userUpdates) > 0 {
		userQuery := fmt.Sprintf("UPDATE users SET %s WHERE id = $%d", strings.Join(userUpdates, ", "), argIdx)
		userArgs = append(userArgs, userID)
		_, err = tx.Exec(userQuery, userArgs...)
		if err != nil {
			middleware.SendDatabaseError(c, "Failed to update user details")
			return
		}
	}
	...
	if input.StaffType != nil {
		staffUpdates = append(staffUpdates, fmt.Sprintf("staff_type = $%d", argIdx))
		staffArgs = append(staffArgs, *input.StaffType)
		argIdx++

		// Update role in user_roles as well
		var roleID string
		err = tx.QueryRow(`SELECT id FROM roles WHERE name = $1`, *input.StaffType).Scan(&roleID)
		if err != nil {
			middleware.SendDatabaseError(c, "Failed to find new role")
			return
		}
		_, err = tx.Exec(`UPDATE user_roles SET role_id = $1 WHERE user_id = $2 AND clinic_id = $3`, roleID, userID, clinicID)
		if err != nil {
			middleware.SendDatabaseError(c, "Failed to update user role")
			return
		}
	}
	if input.Department != nil {
		staffUpdates = append(staffUpdates, fmt.Sprintf("department = $%d", argIdx))
		staffArgs = append(staffArgs, *input.Department)
		argIdx++
	}
	if input.JoiningDate != nil {
		staffUpdates = append(staffUpdates, fmt.Sprintf("joining_date = $%d", argIdx))
		staffArgs = append(staffArgs, *input.JoiningDate)
		argIdx++
	}
	if input.Permissions != nil {
		permissionsJSON := fmt.Sprintf(`["%s"]`, strings.Join(input.Permissions, `","`))
		staffUpdates = append(staffUpdates, fmt.Sprintf("permissions = $%d", argIdx))
		staffArgs = append(staffArgs, permissionsJSON)
		argIdx++
	}
	if input.IsActive != nil {
		staffUpdates = append(staffUpdates, fmt.Sprintf("is_active = $%d", argIdx))
		staffArgs = append(staffArgs, *input.IsActive)
		argIdx++
	}

	if len(staffUpdates) > 0 {
		staffUpdates = append(staffUpdates, "updated_at = CURRENT_TIMESTAMP")
		staffQuery := fmt.Sprintf("UPDATE staff SET %s WHERE user_id = $%d AND clinic_id = $%d", strings.Join(staffUpdates, ", "), argIdx, argIdx+1)
		staffArgs = append(staffArgs, userID, clinicID)
		_, err = tx.Exec(staffQuery, staffArgs...)
		if err != nil {
			middleware.SendDatabaseError(c, "Failed to update staff details")
			return
		}
	}
...
func DeactivateStaff(c *gin.Context) {
    ...
    `, userID, clinicID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to deactivate staff")
		return
	}
...
func CreateQueue(c *gin.Context) {
	...
	if input.QueueType == "doctor" && input.DoctorID == nil {
		middleware.SendValidationError(c, "Doctor ID required", "Doctor ID is required for doctor queues")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create queue")
		return
	}
...
func GetQueues(c *gin.Context) {
	...
	rows, err := config.DB.Query(query, args...)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch queues")
		return
	}
	defer rows.Close()
...
func AssignToken(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to get token number")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to assign token")
		return
	}
...
func ReassignToken(c *gin.Context) {
	...
	if newQueueID == "" {
		middleware.SendValidationError(c, "Queue ID required", "New queue ID is required")
		return
	}
    ...
    `, newQueueID, tokenID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to reassign token")
		return
	}

	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		middleware.SendNotFoundError(c, "token")
		return
	}
...
func PauseQueue(c *gin.Context) {
    ...
    `, queueID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to pause queue")
		return
	}

	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		middleware.SendNotFoundError(c, "queue")
		return
	}
...
func ResumeQueue(c *gin.Context) {
    ...
    `, queueID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to resume queue")
		return
	}
...
func CreateMedicine(c *gin.Context) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "unique constraint") {
			middleware.SendValidationError(c, "Medicine code already exists", "Medicine code must be unique")
			return
		}
		middleware.SendDatabaseError(c, "Failed to create medicine")
		return
	}
...
func GetPharmacyInventory(c *gin.Context) {
	...
	rows, err := config.DB.Query(query, args...)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch inventory")
		return
	}
	defer rows.Close()
...
func UpdateMedicineStock(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to update stock")
		return
	}

	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		middleware.SendNotFoundError(c, "medicine")
		return
	}
...
func CreatePharmacyDiscount(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create discount")
		return
	}
...
func CreateLabTest(c *gin.Context) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "unique constraint") {
			middleware.SendValidationError(c, "Test code already exists", "Test code must be unique")
			return
		}
		middleware.SendDatabaseError(c, "Failed to create lab test")
		return
	}
...
func GetLabTests(c *gin.Context) {
	...
	rows, err := config.DB.Query(query, args...)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch lab tests")
		return
	}
	defer rows.Close()
...
func CreateSampleCollector(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create sample collector")
		return
	}
...
func UploadLabResult(c *gin.Context) {
	...
	if !exists {
		middleware.SendError(c, http.StatusUnauthorized, "USER_NOT_AUTHENTICATED", "User not authenticated", "User authentication is required to access this resource", nil)
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to upload lab result")
		return
	}
...
func CreateInsuranceProvider(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create insurance provider")
		return
	}
...
func GetInsuranceProviders(c *gin.Context) {
	...
	rows, err := config.DB.Query(query, args...)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch insurance providers")
		return
	}
	defer rows.Close()
...
func GetDailyStats(c *gin.Context) {
	...
	if clinicID == "" || dateStr == "" {
		middleware.SendValidationError(c, "Missing parameters", "clinic_id and date are required")
		return
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			middleware.SendDatabaseError(c, "Failed to fetch daily stats")
			return
		}
	}
...
func GetDoctorStats(c *gin.Context) {
    ...
    `, clinicID, dateStr)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch doctor stats")
		return
	}
	defer rows.Close()
...
func GetFinancialReport(c *gin.Context) {
	...
	if clinicID == "" || fromDate == "" || toDate == "" {
		middleware.SendValidationError(c, "Missing parameters", "clinic_id, from_date, and to_date are required")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch financial report")
		return
	}
	defer rows.Close()
...
func MergePatients(c *gin.Context) {
	...
	if err != nil || !primaryExists {
		middleware.SendNotFoundError(c, "primary patient")
		return
	}
	...
	if err != nil || !duplicateExists {
		middleware.SendNotFoundError(c, "duplicate patient")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge appointments")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge clinic assignments")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge queue tokens")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge lab orders")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge pharmacy billing")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge insurance records")
		return
	}
    ...
    `, input.PrimaryPatientID, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to merge insurance claims")
		return
	}
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to clean up duplicate clinic assignments")
		return
	}
    ...
    `, input.DuplicatePatientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to deactivate duplicate patient")
		return
	}

	// Commit transaction
	if err = tx.Commit(); err != nil {
		middleware.SendDatabaseError(c, "Failed to commit merge operation")
		return
	}
...
func GetPatientHistory(c *gin.Context) {
    ...
    `, patientID)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch lab history")
		return
	}
	defer labRows.Close()
...
func CreateFeeStructure(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create fee structure")
		return
	}
...
func GetFeeStructures(c *gin.Context) {
	...
	rows, err := config.DB.Query(query, args...)
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to fetch fee structures")
		return
	}
	defer rows.Close()
...
func CreateBillingDiscount(c *gin.Context) {
	...
	if err != nil {
		middleware.SendDatabaseError(c, "Failed to create billing discount")
		return
	}
