...
func AuthMiddleware(db Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := db.QueryContext(ctx, `
			SELECT r.name, ur.organization_id, ur.clinic_id
			FROM users u
		...
		for rows.Next() {
			...
			if err := rows.Scan(&roleName, &orgID, &clinicID); err != nil {
				continue
			}
		...
		if err = rows.Err(); err != nil {
			SendError(c, http.StatusInternalServerError, CodeAuthVerificationError, "Error reading user data",
				"Database error while verifying identity. Please try again.", nil)
			c.Abort()
			return
		}
		...
		c.Set("is_super_admin", rolesMap["super_admin"])
		c.Next()
	}
}
...
func RequireRole(db Database, expectedRoles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rolesIntf, ok := c.Get("user_roles")
		if !ok {
			SendError(c, http.StatusInternalServerError, CodePermissionCheckError, "Failed to check user permissions",
				"Unable to verify user permissions from context. Please try again later", nil)
			c.Abort()
			return
		}
...
func GetUserOrganizationContext(ctx context.Context, db Database, userID string) ([]string, error) {
	rows, err := db.QueryContext(ctx, `
...
func GetUserClinicContext(ctx context.Context, db Database, userID string) ([]string, error) {
	rows, err := db.QueryContext(ctx, `
...
func RequireOrganizationAdmin(db Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orgIDsIntf, _ := c.Get("organization_ids")
		orgIDs := orgIDsIntf.([]string)
...
func RequireClinicAdmin(db Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		clinicIDsIntf, _ := c.Get("clinic_ids")
		clinicIDs := clinicIDsIntf.([]string)
...
func RequireAnyAdmin(db Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, r := range roles {
			if r == "super_admin" || r == "organization_admin" || r == "clinic_admin" {
				hasAdminRole = true
				break
			}
		}
